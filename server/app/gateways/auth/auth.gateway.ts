...
export class AuthGateway implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit {
    ...
    async updateFriends(@ConnectedSocket() socket: Socket) {
        await this.updateIsOnline(socket);
    }
    ...
    handleOnlineMessage(socket: Socket, userOnline: string) {
        const chat: Chat = this.messageManager.createMessage(userOnline, `${userOnline} est en ligne`);
        socket.emit(ChannelEvents.GlobalMessage, { ...chat, tag: MessageTag.Common });
    }

    async updateIsOnline(socket: Socket) {
