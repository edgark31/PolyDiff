  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  ...
  async createSoloRoom(
    ...
    @MessageBody() playerPayLoad: PlayerData
  ) {
    await this.classicModeService.createSoloRoom(
      socket,
      playerPayLoad,
      this.server
    );
  ...
  async createOneVsOneRoom(
    @ConnectedSocket() socket: Socket,
    @MessageBody() playerPayLoad: PlayerData
  ) {
    await this.classicModeService.createOneVsOneRoom(
      socket,
      playerPayLoad,
      this.server
    );
  ...
  async createLimitedRoom(
    @ConnectedSocket() socket: Socket,
    @MessageBody() playerPayLoad: PlayerData
  ) {
    await this.limitedModeService.createLimitedRoom(
      socket,
      playerPayLoad,
      this.server
    );
  ...
  updateRoomOneVsOneAvailability(
    ...
    this.classicModeService.updateRoomOneVsOneAvailability(
      socket.id,
      gameId,
      this.server
    );
  ...
  checkRoomOneVsOneAvailability(
    ...
    this.classicModeService.checkRoomOneVsOneAvailability(
      socket.id,
      gameId,
      this.server
    );
  ...
  deleteCreatedOneVsOneRoom(
    ...
    if (!room) return;
    this.playersListManagerService.cancelAllJoining(
      room.clientGame.id,
      this.server
    );
  ...
  getJoinedPlayerNames(
    ...
    this.playersListManagerService.getWaitingPlayerNameList(
      socket.id,
      gameId,
      this.server
    );
  ...
  updateWaitingPlayerNameList(
    ...
    this.playersListManagerService.updateWaitingPlayerNameList(
      playerPayLoad,
      socket
    );
    const hostId = this.roomsManagerService.getHostIdByGameId(
  ...
  acceptPlayer(
    @ConnectedSocket() socket: Socket,
    @MessageBody() data: { gameId: string; roomId: string; playerName: string }
  ) {
    const acceptedPlayer = this.playersListManagerService.getAcceptPlayer(
      data.gameId,
      this.server
    );
    this.classicModeService.acceptPlayer(
      acceptedPlayer,
      data.roomId,
      this.server
    );
  ...
  checkIfPlayerNameIsAvailable(@MessageBody() playerPayLoad: PlayerData) {
    this.playersListManagerService.checkIfPlayerNameIsAvailable(
      playerPayLoad,
      this.server
    );
  }
  ...
  checkIfAnyCoopRoomExists(
    @ConnectedSocket() socket: Socket,
    @MessageBody() playerPayLoad: PlayerData
  ) {
    this.limitedModeService.checkIfAnyCoopRoomExists(
      socket,
      playerPayLoad,
      this.server
    );
  ...
  processConnection(
    ...
    const canConnect = !Array.from(this.mapSocketWithName.values()).some(
      (value) => value === name
    );
    socket.emit(ConnectionEvents.UserConnectionRequest, canConnect);
    if (canConnect) {
      this.logger.log(
        `Connexion au chat acceptée pour ${name} avec id : ${socket.id}`
      );
      this.mapSocketWithName.set(socket.id, name);
    } else {
      this.logger.log(
        `Connexion au chat refusée pour ${name} avec id : ${socket.id}`
      );
    }
  }
  ...
  processMessage(@MessageBody() dataMessage: ChatMessageGlobal) {
    this.logger.log(
      `Message reçu : ${dataMessage.message} de la part de ${dataMessage.userName}`
    );
